...
export default function FileList({
    ...
    const starredCount = useMemo(() => {
        return files.filter((file) => file.isFavorite && !file.isDeleted).length;
    }, [files]);
    ...
    const handleStarFile = async (fileId: string) => {
        try {
            ...
            setFiles(
                files.map((file) =>
                    file.id === fileId ? { ...file, isStarred: !file.isFavorite } : file
                )
            );
            ...
            addToast({
                title: file?.isFavorite ? "Removed from Starred" : "Added to Starred",
                description: `"${file?.name}" has been ${
                    file?.isFavorite ? "removed from" : "added to"
                } your starred files`,
                color: "success",
            });
        } catch (error) {
    ...
    const handleTrashFile = async (fileId: string) => {
        try {
            ...
            setFiles(
                files.map((file) =>
                    file.id === fileId ? { ...file, isTrash: !file.isDeleted } : file
                )
            );
        ...
        <div className="space-y-6">
            ...
            {activeTab === "all" && (
                <FolderNavigation
                    ...
                    navigateToPathFolderAction={navigateToPathFolder}
                />
            )}
            ...
            <FileActionButtons
                ...
                onEmptyTrashAction={() => setEmptyTrashModalOpen(true)}
            />
                            ...
                            <span className="truncate max-w-[150px] sm:max-w-[200px] md:max-w-[300px]">
                              {file.name}
                                        ...
                                        <TableCell onClick={(e) => e.stopPropagation()}>
                                            <FileActions
                                                file={file}
                                                onStarAction={handleStarFile}
                                                onTrashAction={handleTrashFile}
                                                onDeleteAction={(file) => {
